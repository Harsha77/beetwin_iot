...
const fetchDevices = async () => {
  try {
    const response = await axios.get("/api/method/beetwin_iot.beetwin_iot.api.get_latest_device_data.get_filtered_device_data");
    ...
    if (response.data && response.data.message && response.data.message.status === "success") {
      deviceList.value = response.data.message.data
        ...
        .map((device) => ({
          ...
          timestamp: device["Timestamp"] ? device["Timestamp"].slice(0, 16) : "N/A",
          latestData: {
            pv: device["Pressure (bar)"] ? device["Pressure (bar)"].toString() : "N/A",
            bt: device["Battery (%)"] ? device["Battery (%)"].toString() : "N/A",
            ht: device["Sensor Health"] ? device["Sensor Health"].toString() : "N/A",
            lat: device["Latitude"] ? device["Latitude"].toString() : "N/A",
            long: device["Longitude"] ? device["Longitude"].toString() : "N/A",
            rssi: device["RSSI Value"] ? device["RSSI Value"].toString() : "N/A",
          },
        }));
    } else {

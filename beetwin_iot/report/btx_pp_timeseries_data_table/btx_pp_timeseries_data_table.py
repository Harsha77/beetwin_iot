# ============================================================
# BTX-PP Timeseries Data Table - Backend Logic for Report & APIs
# ------------------------------------------------------------
# This module includes:
# - `execute`: Data source for Frappe report
# - `generate_device_report`: API for tabular data
# - `generate_device_report_for_graph`: API for graph plotting
# ============================================================
...
# ------------------------------------------------------------
# Function: execute
# Purpose : Report query function for Frappe Report Builder.
# It dynamically constructs column headers and retrieves data
# from Device Reading and Device Reading Key-Value tables
# based on date filters and selected IMEI.
# ------------------------------------------------------------
def execute(filters=None):
    # Define report columns
    columns = [
        {"label": "Timestamp", "fieldname": "timestamp", "fieldtype": "Datetime", "width": 200},
        {"label": "Pressure (bar)", "fieldname": "pv", "fieldtype": "Float", "width": 120},
        {"label": "Battery (%)", "fieldname": "bt", "fieldtype": "Int", "width": 100},
        {"label": "Sensor Health", "fieldname": "ht", "fieldtype": "Data", "width": 100},
        {"label": "Latitude", "fieldname": "lat", "fieldtype": "Data", "width": 150},
        {"label": "Longitude", "fieldname": "long", "fieldtype": "Data", "width": 150},
        {"label": "Signal Strength", "fieldname": "rssi", "fieldtype": "Data", "width": 150}
    ]
    ...
    # Override if specific dates are given
    ...
    # Parse and organize result set into structured rows
    data_dict = {}
    for row in readings:
...
# ------------------------------------------------------------
# API: generate_device_report
# Purpose : Returns parsed telemetry data for a device for
# tabular views based on IMEI and date range.
# ------------------------------------------------------------
...
def generate_device_report(device_data, from_date=None, to_date=None):
    ...
    if from_date and to_date:
        ...
        end_date = datetime.strptime(to_date, '%Y-%m-%d').replace(hour=23, minute=59, second=59).strftime('%Y-%m-%d %H:%M:%S')
    ...
    reading_names_tuple = (reading_names[0],) if len(reading_names) == 1 else tuple(reading_names)
...
# ------------------------------------------------------------
# API: generate_device_report_for_graph
# Purpose : Same as above but filtered to exclude empty rows
# Useful for time-series chart visualizations.
# ------------------------------------------------------------
@frappe.whitelist(allow_guest=True)
def generate_device_report_for_graph(device_data, from_date=None, to_date=None):
    ...
    if from_date and to_date:
        start_date = from_date
        end_date = (datetime.strptime(to_date, '%Y-%m-%d') + timedelta(days=1)).strftime('%Y-%m-%d %H:%M:%S')
    ...
    # Filter out empty telemetry rows (no key data)
    sorted_data = [
        entry for entry in sorted_data
        if not (entry["pv"] is None and entry["bt"] is None and entry["ht"] is None)
    ]

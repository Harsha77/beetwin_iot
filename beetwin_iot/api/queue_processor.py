...
# --------------------------------------------
# NEW: Cleanup helper - delete 'Done' records
# --------------------------------------------
@frappe.whitelist()
def cleanup_device_data_queue(batch_size: int = 500):
    """
    Delete up to `batch_size` rows from Device Data Queue that are already Done.
    Intended to be called from scheduler.
    """
    deleted = 0
    ...
    for r in rows:
        try:
            ...
            frappe.log_error(
                ...
                "Device Data Queue cleanup failed"
            )

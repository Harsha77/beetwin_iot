...
# ===========================================================
# API: get_device_diagnostic_data
# -------------------------------
# Fetch telemetry data for the currently logged-in user.
# - System Manager: Gets all devices.
# - Regular user: Devices only from allowed 'Device Group' permissions.
# ===========================================================
@frappe.whitelist(allow_guest=True)
def get_device_diagnostic_data():
    try:
...
# ===========================================================
# Helper Function: filter_valid_telemetry
# -------------------------------
# Filters out unwanted telemetry keys (e.g., those starting with 't').
# Allows only a defined subset of valid telemetry keys.
# ===========================================================
def filter_valid_telemetry(entries):
    valid_keys = {"srno", "rssi", "ht", "ms", "rc", "rst", "sv"}
    telemetry_map = {}
...
# ===========================================================
# API: get_all_device_data
# -------------------------------
# Admin function to fetch telemetry data for all devices.
# - Applies timestamp and key-value filtering.
# - Validates timestamps against current year and excludes bad entries.
# ===========================================================
@frappe.whitelist(allow_guest=True)
    ...
    try:
        ...
        for device in all_devices:
            ...
            telemetry_data = {
                ...
                "Sensor Health": None,
                "RSSI Value": None,
                "Mqtt Server Data Status": None,
                "Retry Count": None,
                "Device Reset Status": None,
            ...
            telemetry_data["Mqtt Server Data Status"] = filtered_values.get("ms")
            telemetry_data["Retry Count"] = filtered_values.get("rc")
            telemetry_data["Device Reset Status"] = filtered_values.get("rst")
            telemetry_data["Device Software Version"] = filtered_values.get("sv")
            ...
            if telemetry_data["Timestamp"] and any(
                ...
                for key in ["Serial Number", "Sensor Health", "RSSI Value", "Mqtt Server Data Status", "Retry Count", "Device Reset Status", "Device Software Version"]
            ):
